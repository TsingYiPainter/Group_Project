...
// Locate the OctreeNode corresponding to the input point from the given OctreeNode.
    ...
    static bool insertPointRecurse( Ptr<OctreeNode>& node,  const Point3f& point, int maxDepth, const OctreeKey &key,size_t depthMask);
    ...
    void getPointRecurse(std::vector<Point3f> &restorePointCloud,unsigned long x_key,unsigned long y_key,unsigned long z_key, Ptr<OctreeNode>& _node,double resolution,Point3f ori);
...
// For Nearest neighbor search.
    ...
    bool insertPointRecurse( Ptr<OctreeNode>& _node,  const Point3f& point, int maxDepth, const OctreeKey &key, size_t depthMask)
    {
        OctreeNode& node = *_node;
    ...
    void getPointRecurse(std::vector<Point3f> &restorePointCloud,unsigned long x_key,unsigned long y_key,unsigned long z_key, Ptr<OctreeNode>& _node,double resolution,Point3f ori){
        OctreeNode node=*_node;
        if(node.isLeaf){
            restorePointCloud.emplace_back((float)(resolution*x_key)+ori.x,(float)(resolution*y_key)+ori.y,(float)(resolution*z_key)+ori.z);
            return;
        }
